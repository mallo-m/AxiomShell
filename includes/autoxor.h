...
public:
    ...
    __forceinline decltype(auto) decrypt(void)
    {
        for (size_t i = 0; i < N; ++i) {
            ...
            if ((unsigned long)_encrypted[i] > 300)
                i += _encrypted[i];
        }
        _encrypted[N] = '\0';
...
# define xprintf(FORMAT_STR, ...) printf(XorStr(FORMAT_STR), ##__VA_ARGS__);
# define xsnprintf(BUFFER, BUFFER_SIZE, FORMAT_STR, ...) snprintf(BUFFER, BUFFER_SIZE, XorStr(FORMAT_STR), ##__VA_ARGS__);
